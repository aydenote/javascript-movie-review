...
async function handleMoreMovies(event, movieService, fetchMoviesCallback) {
  showSkeleton(20);
...
function ContentsContainer(contentTitle, movieList, movieService, fetchMoviesCallback) {
  ...
  if (movieList.movieList.length === 0) {
    ...
    const $moreButton = Button(
      "더 보기",
      "more",
      (event) => handleMoreMovies(event, movieService, fetchMoviesCallback)
    );
    $main.appendChild($moreButton);
  }
  if (movieList.movieList.length < 20 || movieService.currentPage === MAXIMUM_PAGE) {
    removeMoreButton($main);
  }
}
...
async function renderHeader({ title, poster_path, vote_average }) {
  ...
  container == null ? void 0 : container.prepend(logoSearchBar);
}
